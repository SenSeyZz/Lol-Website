      ...
      for (let j = 0; j< participantGame.length  ; j++){
       ...
       //Set all the values to the ones we got from the API
         ...
         if(winOrLoose){
          ...
          var sentence = `
              Match number <strong>${i+1}</strong>, He was playing <strong>${statistiques.name}</strong>. His role was <strong>${statistiques.position.toLowerCase()}.</strong> ${wOrL}<br/>
...
async function usePromise(){
    ...
    await gameStats()
    MatchStats.participants.length = 0
